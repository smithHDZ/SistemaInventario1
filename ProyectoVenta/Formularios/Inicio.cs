...
namespace ProyectoVenta.Formularios
{
    public partial class Inicio : Form
    {
        ...
        {
            InitializeComponent();
            oPermisos = new ProyectoVenta.Modelo.Permisos();
        }
        private void Inicio_Load(object sender, EventArgs e)
        {
            ...
            if (oPermisos.Salidas == 1) {
                btnsalir.Enabled = true;
                btnsalir.Cursor = Cursors.Default;
            }
            if (oPermisos.Entradas == 1)
            {
                btnentradas.Enabled = true;
                btnentradas.Cursor = Cursors.Default;
            }
            if (oPermisos.Productos == 1)
            {
                btnproductos.Enabled = true;
                btnproductos.Cursor = Cursors.Default;
            }
            if (oPermisos.Clientes == 1)
            {
                btnClientes.Enabled = true;
                btnClientes.Cursor = Cursors.Default;
            }
            if (oPermisos.Proveedores == 1)
            {
                btnProveedores.Enabled = true;
                btnProveedores.Cursor = Cursors.Default;
            }
            if (oPermisos.Inventario == 1)
            {
                btnInventario.Enabled = true;
                btnInventario.Cursor = Cursors.Default;
            }
            if (oPermisos.Configuracion == 1)
            {
                btnConfiguracion.Enabled = true;
                btnConfiguracion.Cursor = Cursors.Default;
            }
        }
